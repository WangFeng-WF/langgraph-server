...
工具：使用数据库，数据库信息： Mysql 39.96.210.106  3306  kotl-cloud-prompt  prompts_user  yuce@2025
...
def execute_sql_with_db(sql: str) -> Dict[str, Any]:
    """直接通过数据库执行SQL查询"""
    try:
...
async def query_cloud_prompts(indicator_name: str, indicator_type: str, business_domain: str) -> Dict[str, Any]:
    """
    使用结构化输出的查询函数
    ...
    try:
        ...
        # 步骤3: 执行SQL查询
        db_result = execute_sql_with_db(sql)
